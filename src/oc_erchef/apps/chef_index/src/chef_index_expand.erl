...
make_doc_for_add(Command = #chef_idx_expand_doc{id = Id, type=Type, search_provider=elasticsearch}) ->
    ...
    [jiffy:encode({[{<<"index">>, {[
     ...
     <<"\n">>,
     jiffy:encode({MetaFieldsPL ++ maybe_data_bag_field(elasticsearch, Type)
...
maybe_data_bag_field(Provider, DataBagName) when is_binary(DataBagName) ->
    [{<<"data_bag">>, chef_index:transform_data(Provider, DataBagName)}];
maybe_data_bag_field(_, _) ->
    [].
